...
I grabbed the payload out of the exploit PoC because it wasn't working as-is.  Some more enumeration reveals that this version of snap is not vulnerable to this exploit, but the path to escelation is the same because the user account can run `snap install *` with sudo.
```
